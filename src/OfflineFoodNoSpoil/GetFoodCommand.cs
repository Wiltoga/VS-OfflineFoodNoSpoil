...
/// <summary>
/// Command used for testing purposes. Gives the player a few spoilable items to test the mod
/// </summary>
internal static class GetFoodCommand
{
    ...
    private const string bucket64 = "AAAAAIg1AAABAAAABghjb250ZW50cwcBMAABAAAAuAMAAOgDAAAGD3RyYW5zaXRpb25zdGF0ZQMRY3JlYXRlZFRvdGFsSG91cnO/+T8SJ1KSQAMVbGFzdFVwZGF0ZWRUb3RhbEhvdXJzFoucEFtSkkAMCmZyZXNoSG91cnMBAAAAAADAQgwPdHJhbnNpdGlvbkhvdXJzAQAAAAAAwEIMEXRyYW5zaXRpb25lZEhvdXJzAQAAAHL5Tz0AAAAA";
    ...
    public static void RegisterCommand(ICoreServerAPI api)
    {
        api.ChatCommands.Create()
            ...
            .HandleWith(args =>
            {
                foreach (var slot in args.Caller.Player?.InventoryManager.Inventories.Values
                    .Where(inventory => inventory.ClassName != "creative")
                    .SelectMany(inventory => inventory)
                    .Where(slot => slot.Empty is false) ?? [])
                {
                    var name = slot.Itemstack!.Collectible.Class;
                    var b64 = Convert.ToBase64String(slot.Itemstack.ToBytes());
                    Debugger.Break();
        ...
        api.ChatCommands.Create()
            ...
            .HandleWith(args =>
            {
                ...
                CollectibleObject? bucket = args.Caller.Entity.World.GetBlock("game:woodbucket");
                ...
                ItemStack bucketStack = new(bucket);
                bucketStack.FromBytes(new(new MemoryStream(Convert.FromBase64String(bucket64))));
                bucketStack.StackSize = 1;
                ProcessTree(bucketStack.Attributes, hours);

                args.Caller.Player.Entity.TryGiveItemStack(bucketStack);
